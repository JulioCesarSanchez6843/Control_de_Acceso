...
// Muestra lista de alumnos para una materia y permite eliminar del curso.
void handleStudentsForMateria() {
  ...
  if (users.size() == 0) {
    ...
    // Tabla listando nombre, cuenta, registro y acción (eliminar de materia)
    ...
    for (auto &ln : users) {
      ...
      // Acción: eliminar usuario SOLO de esta materia
      html += "<td>";
    ...
    // Script cliente para filtrar la tabla por nombre y cuenta
    html += "<script>"
...
// GET /students_all
// Muestra todos los alumnos (agrupa por UID) con edición y eliminación total.
void handleStudentsAll() {
  String html = htmlHeader("Alumnos - Todos");
  html += "<div class='card'><h2>Todos los alumnos</h2>";

  // Filtros cliente: Nombre, Cuenta, Materia
  ...
  // Leer archivo y agrupar por UID (un registro por usuario con lista de materias)
  while (f.available()) {
  ...
  if (uids.size()==0) html += "<p>No hay alumnos registrados.</p>";
    ...
    // Tabla con cada usuario, sus materias y acciones (editar, eliminar)
    ...
    for (int i=0;i<(int)uids.size();i++) {
      ...
      // Editar: abre formulario de captura en modo edición y vuelve a students_all
    ...
    // Script cliente para filtrar por Nombre, Cuenta y Materia
    html += "<script>"
...
// Elimina la fila correspondiente (uid + materia) del archivo USERS_FILE.
...
// POST /student_delete
// Borra todas las filas con el UID (elimina usuario completamente).
